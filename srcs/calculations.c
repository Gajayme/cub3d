...
/*   Updated: 2022/10/17 11:47:33 by georgii          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ray_pos_dir(int x, t_geom *geom)
{
	...
	geom->delta_dist_x = (geom->ray_dir_x == 0) ? 1e30 : fabs(1 / geom->ray_dir_x);
	geom->delta_dist_y = (geom->ray_dir_y == 0) ? 1e30 : fabs(1 / geom->ray_dir_y);
	geom->hit = 0;
}
...
void	perform_dda(t_geom *geom)
{
	while (geom->hit == 0)
	{
		...
		if (worldMap[geom->map_x][geom->map_y] > 0)
			geom->hit = 1;
	}
	if (geom->side == 0)
